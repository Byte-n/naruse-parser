...
function getNewAcorn() {
    ...
    var options: { [x: string]: any; locations?: any; program?: any; sourceFile?: any; ecmaVersion?: any; ranges?: any; allowHashBang?: any; directSourceFile?: any; strictSemicolons?: any; allowImportExportEverywhere?: any; allowReturnOutsideFunction?: any; preserveParens?: any; allowTrailingCommas?: any; forbidReserved?: any; }, input: string, inputLen: number, sourceFile: null;

    acorn.parse = function (inpt: any, opts: any) {
        input = String(inpt);
    ...
    acorn.parseExpressionAt = function (inpt: any, pos: any, opts: any) {
        input = String(inpt);
        inputLen = input.length;
        setOptions(opts);
        initTokenState(pos);
    ...
    function setOptions(opts: { [x: string]: any; }) {
        options = {};
    ...
    acorn.tokenize = function (inpt: any, opts: any) {
        input = String(inpt);
        inputLen = input.length;
        setOptions(opts);
        initTokenState();
        ...
        getToken.jumpTo = function (pos: number, exprAllowed: any) {
            tokPos = pos;
            if (options.locations) {
                tokCurLine = 1;
    ...
    var tokStart: number, tokEnd: any;

    // When `options.locations` is true, these hold objects
    // containing the tokens start and end line/column pairs.

    var tokStartLoc: any, tokEndLoc: any;
    ...
    var tokType: { keyword?: any; isLoop?: any; isAssign?: any; binop?: any; prefix?: any; isUpdate?: any; postfix?: any; atomValue?: any; type?: string; beforeExpr?: boolean; }
    var tokVal: any;
    ...
    var tokContext: any[], tokExprAllowed: boolean;
    ...
    var lastStart: any, lastEnd: any, lastEndLoc: any;
    ...
    var inFunction: boolean, inGenerator: boolean, labels: { kind: string | null; name?: any; }[], strict: boolean;
    ...
    class ParserError extends SyntaxError {
        ...
        constructor(message: string, pos: number, loc: any, raisedAt: number) {
            super(message);
            this.pos = pos;
            this.loc = loc;
    ...
    function raise(pos: number | Error, message?: string | undefined) {
    ...
    var isIdentifierStart = function (code: number) {
    ...
    var isIdentifierChar = function (code: number) {
    ...
    function Position(line: any, col: number) {
        this.line = line;
        this.column = col;
    }

    Position.prototype.offset = function (n: any) {
    ...
    function initTokenState(pos?: number) {
        if (pos) {
            tokPos = pos;
    ...
    function braceIsBlock(prevType: { type?: string; beforeExpr?: boolean; keyword?: string; }) {
        var parent;
    ...
    function finishToken(type: { type?: string; beforeExpr?: any; keyword?: any; postfix?: boolean; prefix?: boolean; isUpdate?: boolean; }, val?: string | number | null | undefined) {
        tokEnd = tokPos;
    ...
    function skipLineComment(startSkip: number) {
    ...
    function readToken_plus_min(code: any) { // '+-'
        var next = input.charCodeAt(tokPos + 1);
        if (next === code) {
    ...
    function getTokenFromCode(code: any) {
        switch (code) {
    ...
    function readInt(radix: number | string, len?: number | null | undefined) {
        var start = tokPos,
            total = 0;
    ...
    function readRadixNumber(radix: string | number) {
        tokPos += 2; // 0x
    ...
    function readEscapedChar() {
        var ch = input.charCodeAt(++tokPos);
        var octal: any = /^[0-7]+/.exec(input.slice(tokPos, tokPos + 3));
    ...
    function SourceLocation(this: any) {
    ...
    function finishNode(node: any, type: string) {
        node.type = type;
        node.end = lastEnd;
        if (options.locations)
    ...
    function finishNodeAt(node: { loc: { end: any; }; type: any; end: any; range: any[]; }, type: string, pos?: any) {
        if (options.locations) {
            node.loc.end = pos[1];
            pos = pos[0];
        }
    ...
    function eat(type: { type?: string; beforeExpr?: boolean; isAssign?: boolean; keyword?: string; binop?: number; }) {
        if (tokType === type) {
            next();
            return true;
        } else {
    ...
    function isContextual(name: string) {
        return tokType === _name && tokVal === name;
    }

    // Consumes contextual keyword if possible.

    function eatContextual(name: string) {
        return tokVal === name && eat(_name);
    }

    // Asserts that following token is given contextual keyword.

    function expectContextual(name: string) {
    ...
    function expect(type: { type?: string; beforeExpr?: boolean; keyword?: string; isLoop?: boolean; }) {
        eat(type) || unexpected();
    }
    ...
    function has(obj: any, propName: PropertyKey) {
    ...
    function toAssignable(node: { type: string; properties: string | any[]; elements: any; operator: string; left: { end: any; }; start: any; }, isBinding?: boolean) {
    ...
    function toAssignableList(exprList: string | any[], isBinding: boolean) {
        if (exprList.length) {
    ...
    function parseSpread(refShorthandDefaultPos: any) {
        var node = startNode();
        next();
    ...
    function parseBindingList(close: { type: string; }, allowEmpty: boolean) {
        var elts = [],
            first = true;
        while (!eat(close)) {
    ...
    function parseMaybeDefault(startPos?: number | any[], left?: any) {
    ...
    function checkFunctionParam(param: { type: any; name: string; start: any; properties: string | any[]; elements: string | any[]; argument: any; }, nameHash: { [x: string]: boolean; }) {
        switch (param.type) {
            case Identifier:
    ...
    function checkPropClash(prop: { key: any; kind: string; }, propHash: { [x: string]: { init: boolean; get: boolean; set: boolean; }; }) {
    ...
    function parseTopLevel(node: { body: any[]; }) {
        var first = true;
    ...
    function parseStatement(declaration: boolean, topLevel?: boolean | undefined) {
    ...
    function parseBreakContinueStatement(node: { label: { name: any; } | null; start: any; }, keyword: string) {
    ...
    function parseDebuggerStatement(node: any) {
        next();
        semicolon();
        return finishNode(node, DebuggerStatement);
    }

    function parseDoStatement(node: { body: any; test: any; }) {
        next();
    ...
    function parseForStatement(node: any) {
        next();
        labels.push(loopLabel);
        expect(_parenL);
    ...
    function parseFunctionStatement(node: any) {
        next();
        return parseFunction(node, true);
    }

    function parseIfStatement(node: { test: any; consequent: any; alternate: any; }) {
        next();
        node.test = parseParenExpression();
    ...
    function parseReturnStatement(node: { argument: null; }) {
    ...
    function parseSwitchStatement(node: { discriminant: any; cases: any[]; }) {
        next();
    ...
    function parseThrowStatement(node: { argument: any; }) {
        next();
    ...
    function parseTryStatement(node: { block: any; handler: null; guardedHandlers: any[]; finalizer: any; start: any; }) {
        next();
    ...
    function parseVarStatement(node: any, kind: any) {
        next();
    ...
    function parseWhileStatement(node: { test: any; body: any; }) {
        next();
        node.test = parseParenExpression();
        labels.push(loopLabel);
        node.body = parseStatement(false);
    ...
    function parseEmptyStatement(node: any) {
        next();
        return finishNode(node, EmptyStatement);
    }

    function parseLabeledStatement(node: { body: any; label: any; }, maybeName: string, expr: { start: any; }) {
        for (var i = 0; i < labels.length; ++i)
    ...
    function parseExpressionStatement(node: { expression: any; }, expr: any) {
    ...
    function parseBlock(allowStrict?: boolean | undefined) {
        var node = startNode(),
            first = true,
            oldStrict;
        node.body = [];
    ...
    function parseFor(node: { init: any; test: any; update: any; body: any; }, init: null) {
        node.init = init;
        expect(_semi);
    ...
    function parseForIn(node: { left: any; right: any; body: any; }, init: any) {
    ...
    function parseExpression(noIn?: boolean, refShorthandDefaultPos?: { start: number }) {
    ...
    function parseMaybeAssign(noIn?: boolean, refShorthandDefaultPos?: { start: any; }) {
    ...
    function parseMaybeConditional(noIn: any, refShorthandDefaultPos: { start: any; }) {
        var start = storeCurrentPos();
    ...
    function parseExprOps(noIn: any, refShorthandDefaultPos: { start: any; }) {
        var start = storeCurrentPos();
    ...
    function parseExprOp(left: any, leftStart: any, minPrec: number, noIn: any) {
    ...
    function parseMaybeUnary(refShorthandDefaultPos?: { start: any; } | undefined) {
        if (tokType.prefix) {
            var node = startNode(),
    ...
    function parseExprSubscripts(refShorthandDefaultPos?: { start: any; } | undefined) {
        var start = storeCurrentPos();
    ...
    function parseSubscripts(base: any, start: any, noCalls?: boolean | undefined) {
        if (eat(_dot)) {
            var node = startNodeAt(start);
            node.object = base;
    ...
    function parseExprAtom(refShorthandDefaultPos?: { start: any; }) {
        switch (tokType) {
            case _this:
                var node = startNode();
                next();
    ...
    function parseObj(isPattern?: boolean, refShorthandDefaultPos?: { start: any; }) {
        var node = startNode(),
            first = true,
            propHash = {};
        node.properties = [];
    ...
    function parsePropertyName(prop: { computed: boolean; key: any; }) {
        if (options.ecmaVersion >= 6) {
            if (eat(_bracketL)) {
                prop.computed = true;
    ...
    function initFunction(node: { id: null; generator: boolean; expression: boolean; }) {
        node.id = null;
        if (options.ecmaVersion >= 6) {
    ...
    function parseFunction(node: any, isStatement: boolean, allowExpressionBody?: undefined) {
        initFunction(node);
        if (options.ecmaVersion >= 6) {
    ...
    function parseMethod(isGenerator: boolean | undefined) {
        var node = startNode();
        initFunction(node);
        expect(_parenL);
        node.params = parseBindingList(_parenR, false);
    ...
    function parseArrowExpression(node: any, params: any[]) {
        initFunction(node);
    ...
    function parseFunctionBody(node: { body: { body: string | any[]; }; expression: boolean; generator: any; id: any; params: string | any[]; }, allowExpression: boolean) {
    ...
    function parseClass(node: { id: any; superClass: any; body: any; }, isStatement: boolean) {
        next();
    ...
    function parseExprList(close: { type: string; }, allowTrailingComma: boolean, allowEmpty?: boolean, refShorthandDefaultPos?: { start: number; }) {
        var elts = [],
            first = true;
        while (!eat(close)) {
            if (!first) {
    ...
    function parseImport(node: { specifiers: any[]; source: any; kind: string; }) {
        next();
        // import '...';
        if (tokType === _string) {
            node.specifiers = [];
